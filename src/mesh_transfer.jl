...
function get_global_gauss_point_coordinates{dim, T}(fe_CellValues::CellValues{dim, T}, mesh)
    n_qpoints = length(points(get_quadrule(fe_CellValues)))
    n_elements = size(mesh.topology, 2)
    n_basefuncs = n_basefunctions(getfunctionspace(fe_CellValues))
    ...
    for element_id in 1:n_elements
        ...
        reinit!(fe_CellValues, x_vec)
        for q_point in 1:n_qpoints
            x_glob[q_point, element_id] = function_value(fe_CellValues, q_point, x_vec)
        end
    end
    return x_glob
end
...
# Interpolates finite element CellValues from a mesh to another set of points
function interpolate_to{QD <: QuadratureData}(solution_coarse_nodes, quad_data_coarse::AbstractVector{QD}, coarse_mesh,
