...
function create_mesh_and_dofhandler(inputfile, dim, nslips, probtype)
    ...
    geomesh = Grid(condensed_mesh, bulk_element_name)
    dh = ViscoCrystalPlast.add_grad_dofs!(geomesh, duplicated_nodes, nslips, probtype)
    return geomesh, dh
end

function get_RVE_boundary_nodes(mesh::Grid)
    nodes_on_RVE_edge = Int[]

    for (name, nodes) in getnodesets(mesh)
...
function get_grain_boundary_nodes{dim}(mesh::Grid{dim})
    grain_name = dim == 2 ? "face" : "poly"
    nodes_grain = [Int[] for i in 1:length(elementsets(mesh))]

    curr_set = 0
    for (name, elements) in getelementsets(mesh)
        !startswith(name, grain_name) && continue
        p = parse(Int, name[5:end])
        for element in elements
            for v in getcells(mesh, element)
                push!(nodes_grain[p], v.nodes)
            end
        end
    end
    ...
    n_grains = zeros(Int, getnnodes(mesh))
...
function create_element_to_grain_map{dim}(mesh::Grid{dim})
    grain_name = dim == 2 ? "face" : "poly"
    poly = zeros(Int, getncells(mesh))
    for (name, elements) in getcellsets(mesh)
...
function add_grad_dofs!{dim}(mesh::Grid{dim}, duplicated_nodes::Vector{Vector{Int}}, nslips, probtype)
    @assert probtype in (:dual, :primal)

    dh = DofHandler(mesh)
    push!(dh, :u, dim)
    if probtype == :primal
        push!(dh, [Symbol("slip_", i) for i in 1:nslips], (ones(Int, nslips)...))
    else
        push!(dh, [Symbol("xi_perp_", i) for i in 1:nslips], (ones(Int, nslips)...))
        if dim == 3
            push!(dh, [Symbol("xi_o_", i) for i in 1:nslips], (ones(Int, nslips)...))
        end
    end

    all_nodes = 1:getnnodes(mesh)
    nodes_without_slip = setdiff(all_nodes, getnodeset(mesh, "slip_boundary"))
...
function condense_mesh(mesh, old_to_new, dim)
    ...
    new_nodes = AbaqusNodes(new_node_numbers, reshape(new_coordinates, (3, length(new_coordinates) ÷ 3)))
    new_elements = Dict(bulk_element_name => AbaqusElements(new_element_numbers, new_topology))

    return AbaqusMesh(new_nodes, new_elements, new_node_sets, new_element_sets), duplicated_nodes
end
