function solve_problem{dim}(problem::AbstractProblem, mesh, dofs, bcs, fe_values::FEValues{dim}, mp, timesteps, boundary_f, exporter)
    ...
    #lin_problem = NewtonProblem(K, length(dofs.dof_ids), length(free))
    ...
    #quad_data = create_quadrature_data(problem, Dim{dim}, get_quadrule(fe_values), length(mp.angles), size(mesh.topology,2))
    ...
    for (nstep, t) in enumerate(timesteps[2:end])
        ...
        #exporter(time, primary_field, mss)
    end
...
function assemble!{dim}(K::SparseMatrixCSC, primary_field::Vector, prev_primary_field::Vector,
   ...
   # chunk_size = 20, needs to parameterize assemble according to chunk_size?
    ...
    Ke! = ForwardDiff.jacobian(fe, mutates = true, chunk_size = 12)
