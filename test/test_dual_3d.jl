...
srand(1234)
nslip = 2
uu = rand(12 + 2*4 * nslip);
uu[ViscoCrystalPlast.compute_ξdofs(3, 4, 1,2,1, :ξ⟂)] = 0.0
uu[ViscoCrystalPlast.compute_ξdofs(3, 4, 1,2,1, :ξo)] = 0.0
uu[ViscoCrystalPlast.compute_ξdofs(3, 4, 1,2,2, :ξ⟂)] = 0.0
uu[ViscoCrystalPlast.compute_ξdofs(3, 4, 1,2,2, :ξo)] = 0.0
prev_field = zero(uu)
...
mss = [ViscoCrystalPlast.CrystPlastDualQD(nslip, Dim{3}) for i = 1:n_qpoints]
temp_mss = [ViscoCrystalPlast.CrystPlastDualQD(nslip, Dim{3}) for i = 1:n_qpoints]

fe_u = [zero(Vec{3, Float64}) for i in 1:4]
