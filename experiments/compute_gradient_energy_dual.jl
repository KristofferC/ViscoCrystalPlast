...
function startit()
    ...
    for l in 0.1:0.1:1.0
        ...
        dofs_fine = ViscoCrystalPlast.add_dofs(mesh_fine, [:u, :v, :ξ⟂1, :ξ⟂2])
        bcs_fine = ViscoCrystalPlast.DirichletBoundaryConditions(dofs_fine, mesh_fine.boundary_nodes, [:u, :v])
        pvd_fine = paraview_collection("vtks/shear_dual_fine")
        timestep_fine = 0
        ...
        sol_fine, mss_fine = ViscoCrystalPlast.solve_problem(ViscoCrystalPlast.DualProblem(), mesh_fine, dofs_fine, bcs_fine, fe_values, mp, times,
    ...
    save("dataframes/dual_l_study.jld", "df", df_l_study)

    print_timer()
    #save("dataframes/dual_data_frame.jld", "df", df)
    return df
end
...
function total_slip{T}(mesh, dofs, u::Vector{T}, mss, fev, nslip, mp)
    ...
    for i in 1:size(mesh.topology, 2)
        ...
        reinit!(fev, x_vec)
        for α in 1:nslip
            gd = ViscoCrystalPlast.g_dofs(dim, nnodes, ngradvars, nslip, α)
            ξos[α] = ug[gd]
        end
        ...
        for q_point in 1:length(points(get_quadrule(fev)))
            for α = 1:nslip
                ...
                tot_grad_en += 0.5 / mp.lα^2 * (ξo^2 / mp.Ho) * detJdV(fev, q_point)
