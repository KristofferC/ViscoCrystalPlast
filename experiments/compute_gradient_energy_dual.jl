...
function startit{dim}(::Dim{dim})
    ...
    for l in 0.1:0.1:0.1
        ...
        if dim == 2
            ...
            dofs_fine = ViscoCrystalPlast.add_dofs(mesh_fine, [:u, :v, :w, :ξ⟂1, :ξ⟂2, :ξo1, :ξo2])
        end
...
function total_slip{T, dim}(mesh, dofs, u::Vector{T}, mss, fev::FEValues{dim}, nslip, mp)
    ...
    for i in 1:size(mesh.topology, 2)
        ...
        for α in 1:nslip
            if dim == 2
                ξ⟂_node_dofs = ViscoCrystalPlast.γ_dofs(dim, nnodes, ngradvars, nslip, α)
            else
